...
def image_a_b_gen(batch_size):
    for batch in datagen.flow(Xtrain, batch_size=batch_size):
        ...
        X_batch = X_batch / 128.0  # Normalización de X_batch
...
Xtest = Xtest / 128.0  # Normalización de Xtest
Xtest = tf.convert_to_tensor(Xtest, dtype=tf.float32)

Ytest = rgb2lab(1.0/255*X[split:])[:,:,:,1:]
Ytest = Ytest / 128.0  # Normalización de Ytest
...
with device:
    ...
    history = model.fit_generator(
        ...
        validation_steps=samples_test//batch_size
    )
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('/home/alumne/xnap-project-ed_group_05/experiments/wandb/norma_africa/loss_wb.png')
plt.show()
...
for i in range(len(output)):
    ...
    cur[:,:,0] = color_me[i][:,:,0]
    cur[:,:,1:] = output[i]
    imsave("/home/alumne/xnap-project-ed_group_05/experiments/wandb/norma_africa/img_"+str(i)+".png", lab2rgb(cur))
